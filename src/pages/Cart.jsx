...
export default function Cart({ cart, onRemove, onUpdateQty }) {
    const total = cart.reduce((acc, item) => acc + (item.wholesale_price * item.quantity), 0);

    const generateWhatsAppLink = () => {
        const phone = "5215512345678"; // Replace with actual number
        const itemsList = cart.map(item => `• (${item.quantity}pz) ${item.sku} - ${item.name}`).join('%0A');
                ...
                <div className="flex-1 space-y-4">
                    ...
                    {cart.map(item => (
                        <div key={item.id} className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 flex gap-4 items-center">
                            <img src={item.image_url || 'https://via.placeholder.com/100'} className="w-20 h-20 object-contain bg-gray-50 rounded" />
                            <div className="flex-1">
                                <h4 className="font-bold text-gray-900 line-clamp-1">{item.name}</h4>
                                <div className="text-xs text-gray-500">SKU: {item.sku}</div>
                                <div className="text-sm font-bold text-[#1A1A1A] mt-1">${item.wholesale_price}</div>
                            </div>
